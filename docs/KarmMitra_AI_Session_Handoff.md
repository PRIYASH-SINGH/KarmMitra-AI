...
1. **End-to-End LTI Flow Verification:** Validate full flow from mock iGOT launch -> Role Triage -> RAG Assessment -> AGS Grade Passback. (Mock iGOT is ready on 9000, Gateway is ready on 8000).
2. **Admin UI Wiring:** Complete the Admin UI dashboard (Member 5) to monitor department-wide competency gaps and passback rates via the `/api/v1/admin/metrics` endpoint.
3. **Bhashini Integration:** Inject `BHASHINI_API_KEY` and `BHASHINI_USER_ID` into `.env` (once approved by DPI portal) and verify the live Hindi translation toggle in the Learner UI.
4. **Server-Side Grading Cache:** `assessment.py`'s `POST /submit` still trusts the client's `correct_answers`. Needs to be fixed to cache the generated answer key server-side.
