...
2. **RAG Trigger (Member 4 -> 1):** Learner UI calls `POST /api/v1/rag/generate?competency_code=X&competency_name=Y` to Gateway.
3. **Sovereign AI (Member 1 -> 2):** Gateway routes request to Member 2's local `vLLM/Ollama` running on `host.docker.internal:11434`.
4. **Resilience / Fallback (Member 2):** If the LLM is offline or unreachable, the `SovereignAIEngine` gracefully returns 3 synthetic, zero-hallucination MoSPI MCQs to preserve UI flow.
5. **Validation (Member 1):** Gateway validates Member 2's response against the Pydantic schema to catch hallucinations/malformed JSON.
6. **Render (Member 1 -> 4):** Verified MCQ JSON returned to React frontend.
